...
jQuery(document).ready(function () {
        ...
        $jcrop_cut_btn.click(function () {
            ...
            if ($cut_value != '') {
                ...
                $.ajax({
                    ...
                    .fail(function (res) {
                        pageLoader('hide');
                        tips($(res.responseText).find('h1').text() || '请求失败~', 'danger');
                    });
            } else {
                tips('请选择要裁剪的大小', 'warning');
            }
        });
    ...
    $('.js-ueditor').each(function () {
        ...
        console.log(Quick.ueditor_upload_url)
    });
    ...
    $('.js-upload-file,.js-upload-files').each(function () {
        ...
        // 允许上传的后缀
        var $ext = $input_file.attr('ext');
        // 文件限制大小
        var $size = $input_file.attr('size');
        // 文件列表
        ...
        uploader.on('error', function (type) {
            switch (type) {
                case 'Q_TYPE_DENIED':
                    tips('文件类型不正确，只允许上传后缀名为：' + $ext + '，请重新上传！', 'danger');
                    break;
                case 'F_EXCEED_SIZE':
                    tips('文件不得超过' + ($size / 1024) + 'kb，请重新上传！', 'danger');
                    break;
            }
        });
    ...
    // 图片上传
    $('.js-upload-level,.js-upload-images').each(function () {
        ...
        var thumbnailHeight = height ? height : 100;
        ...
        var uploader = WebUploader.create({
            ...
            accept: {
                title: 'Images',
                extensions: $ext,
                mimeTypes: 'level/jpg,level/jpeg,level/bmp,level/png,level/gif'
            },
            // 自定义参数
            formData: {
                dir: $dir,
        ...
        uploader.on('error', function (type) {
            switch (type) {
                case 'Q_TYPE_DENIED':
                    tips('图片类型不正确，只允许上传后缀名为：' + $ext + '，请重新上传！', 'danger');
                    break;
                case 'F_EXCEED_SIZE':
                    tips('图片不得超过' + ($size / 1024) + 'kb，请重新上传！', 'danger');
                    break;
            }
        });
    ...
    $('.select-linkage').change(function () {
        ...
        if (value != '') {
            ...
            $.ajax({
                ...
                .fail(function (res) {
                    pageLoader('hide');
                    tips($(res.responseText).find('h1').text() || '数据请求失败~', 'danger');
                });
        }
    });
