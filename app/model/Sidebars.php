...
class Sidebars extends Model
{
    ...
    {
        $cache_tag .= '_role_' . session('user_auth.role_id');
    ...
    public static function getSidebarMenu($id = '', $module = '', $controller = '')
    {
        ...
        $cache_tag = strtolower('_sidebar_menus_' . $module . '_' . $controller) . '_role_' . session('user_auth.role_id');
        $menus = cache($cache_tag);
    ...
    public static function getLocation($id = '', $del_last_url = false, $check = true)
    {
        ...
        if ($id != '') {
            ...
            $cache_name = 'location_/' . $controller . '_' . $action;
        }
        ...
        if (!$location) {
            $map[] = ['pid', '<>', 0];
            $map[] = ['url_value', '=', strtolower( '/' . trim(preg_replace("/[A-Z]/", "_\\0", $controller), "_") . '/' . $action)];
            ...
            if ($check && empty($location)) {
                return ['error' => 1, 'msg' => '获取不到当前节点地址，可能未添加节点'];
            }
