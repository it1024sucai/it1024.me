...
{//类定义开始
    ...
    private $config
        = array(
            ...
            'generator'          => 'xiaohongyy.com RSS Generator',        // 生成器
            'category'           => '',
        );
    ...
    // $descrīption $pubDate
    /**************************************************************************/
    function AddItem($title, $link, $descrīption, $pubDate, $guid, $author = "XiaoYu", $category)
    {
        $this->items[] = array(
            'title'    => $title, 'link' => $link, 'descrīption' => $descrīption, 'pubDate' => $pubDate,
    ...
    function BuildRSS()
    {
        $s = "<?xml version=\"1.0\" encoding=\"{$this->encoding}\"?>\r\n<rss version=\"{$this->rssVer}\" xmlns:atom=\"http://www.w3.org/2005/Atom\">\n";
        $s .= "\t<channel>\r\n";
        $s .= "\t\t<title><![CDATA[{$this->channelTitle}]]></title>\r\n";
        $s .= "\t\t<link><![CDATA[{$this->channelLink}]]></link>\r\n";
        $s .= "\t\t<description><![CDATA[{$this->channelDescrīption}]]></description>\r\n";
        ...
        for ($i = 0; $i < count($this->items); $i++) {
            ...
            $s .= "\t\t\t<description><![CDATA[{$this->items[$i]['descrīption']}]]></description>\r\n";
